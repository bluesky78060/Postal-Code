...
function buildSectionXml(rows, options = {}) {
  ...
  // 모든 페이지의 테이블을 하나의 섹션에 포함 (페이지 나누기 적용)
  const tablesWithBreaks = pages.map((pageItems, pageIndex) => {
    const table = pageTable(pageItems, pageIndex);
    // 두 번째 테이블부터 페이지 나누기 추가
    if (pageIndex > 0) {
      const pageBreakId = 3121190098 + pageIndex * 100;
